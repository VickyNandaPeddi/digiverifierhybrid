...
@Component({
  ...
  templateUrl: './candidate-tracker.component.html',
  styleUrls: ['./candidate-tracker.component.scss']
})
export class CandidateTrackerComponent implements OnInit {
  pageTitle = 'Candidate Tracker Report';
  fromDate: any;
  toDate: any;
  setfromDate: any;
  settoDate: any;
  ...
  onfromDate(event:any) {
   ...
   onSubmitFilter(trackerFilter:FormGroup){
    ...
    // if (this.trackerFilter.valid) {
      ...
      this.customers.getCustomerStatusTrackerReport({
        ...
        (data: any) => {
          ...
          link.download = 'customer_status_tracker.xlsx';
          link.click();
        },
        error => {

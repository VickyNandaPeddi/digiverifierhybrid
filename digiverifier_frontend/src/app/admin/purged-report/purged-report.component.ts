...
export class PurgedReportComponent implements OnInit {
  ...
  downloadInterimReport(candidate: any,reportStatus: any) { 
    ...
    if(candidate.candidateCode) {
      this.orgadmin.getPreSignedUrlByCandidateCode(candidate.candidateCode,reportStatus,false).subscribe((url: any)=>{ 
  ...
  oldCandidatePurge() {
    this.orgadmin.oldCandidatePurge(this.authService.getOrgID()).subscribe((data: any)=>{ 

      if(data.outcome) {
        Swal.fire({
          title: 'Purge Process Started Successfully.',
          icon: 'success',
        });
      } else {
        Swal.fire({
